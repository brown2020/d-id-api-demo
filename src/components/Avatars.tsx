    ...
    <div className="relative p-2">
      ...
      <div className="p-4 flex flex-col gap-4 w-full">
        <div>
          ...
          <button type="button" onClick={createNewTalkingPhoto} className="group/avatar cursor-pointer relative border-2 border-gray-300 hover:drop-shadow-2xl transition-colors hover:-translate-y-2 ease-in-out duration-300 isolate flex flex-col justify-end overflow-hidden rounded-2xl px-8 pb-8 pt-40 lg:pt-40 xl:pt-44 2xl:pt-52 mx-auto w-full">
            ...
            {filteredTalkingPhotos.map((photo) => (
              <AvatarCard avatar={photo} key={photo.talking_photo_id} id={photo.talking_photo_id} edit={() => openForm(photo)} />
            ))}
          </ul>
        </div>
        <div>
          ...
          <ul className="grid min-[450px]:grid-cols-2 grid-cols-1 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {filteredTemplateTalkingPhotos.map((photo) => (
              <AvatarCard avatar={photo} key={photo.talking_photo_id} id={photo.talking_photo_id} />
            ))}
          </ul>
        </div>
      </div>
