...
import { CreateVideoForm, DIDTalkingPhoto, Emotion } from "@/types/did";
...
export default function CreateVideo() {
    ...
    const { handleSubmit, control, formState, reset, watch, setValue, getValues } = useForm<CreateVideoForm>({ mode: 'all' });

    useEffect(() => {
        setValue('talking_photo_id', selectedAvatar ? selectedAvatar.talking_photo_id : '')
        setValue('voice_id', selectedAvatar && selectedAvatar.voiceId ? selectedAvatar.voiceId : '')
    }, [selectedAvatar])
    ...
    const [activeStep, setActiveStep] = useState(steps[0].code)
    ...
    return <div className="px-4 max-h-full h-full flex flex-col">
        ...
        <div className="py-4 px-1 grow overflow-hidden">
            <div className="flex w-full max-h-full h-full gap-4 overflow-auto">
                ...
                {
                    selectedAvatar ? <div className="grow bg-gray-50 rounded-lg p-4">
                                        ...
                                        render={({ field }) => (
                                            <div>
                                                ...
                                                <ul className="items-center w-full text-sm font-medium border-gray-200 rounded-lg sm:flex ">
                                                    {
                                                        emotions.map((emotion, index) => <li key={index} onClick={() => { setValue('emotion', emotion.code) }} className={`cursor-pointer w-full border-gray-200 sm:border-r first:rounded-l-lg last:rounded-r-lg ${field.value == emotion.code ? 'bg-slate-600 text-white' : 'bg-white border text-gray-900'}`}>
